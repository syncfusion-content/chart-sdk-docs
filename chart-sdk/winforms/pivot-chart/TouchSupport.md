...
The pivot chart provides the swipe scrolling and panning support in the horizontal direction.

![WinForms PivotChart with touch swiping](TouchSupport_images/PivotChart_img1.png)

## Touch zooming

Touch zooming in both the x-axis and y-axis can be enabled by setting the [EnableXZooming](https://help.syncfusion.com/cr/windowsforms/Syncfusion.Windows.Forms.PivotChart.PivotChart.html#Syncfusion_Windows_Forms_PivotChart_PivotChart_EnableXZooming) and [EnableYZooming](https://help.syncfusion.com/cr/windowsforms/Syncfusion.Windows.Forms.PivotChart.PivotChart.html#Syncfusion_Windows_Forms_PivotChart_PivotChart_EnableYZooming) properties to `true`. The default value of `EnableXZooming` is `true` and the default value of `EnableYZooming` is `false`.
...
![WinForms PivotChart with touch zooming](TouchSupport_images/PivotChart_img2.png)

### Disabling X and Y axes zooming

The zooming can be disabled in the x-axis and y-axis by setting the `EnableXZooming` and `EnableYZooming` properties to `false`.
