...
To denote the axis values with temperature units, add `°C` as a suffix to each label. This can be achieved by setting `{value}°C` as the value of the [`format`](https://ej2.syncfusion.com/react/documentation/api/linear-gauge/labelModel#format-string) property in the [`labelStyle`](https://ej2.syncfusion.com/react/documentation/api/linear-gauge/axis#labelstyle-labelmodel) object of the axis. Here, `{value}` acts as a placeholder for each axis label. Other common formats include `{value}%` for percentages and `${value}` for currency values.

To change the pointer value from the default value of the gauge, set the [`value`](https://ej2.syncfusion.com/react/documentation/api/linear-gauge/pointer#value-number) property of a `PointerDirective` inside the `PointersDirective` collection on the axis.
