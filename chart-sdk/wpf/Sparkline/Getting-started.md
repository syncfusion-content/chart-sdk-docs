---
layout: post
title: Getting Started with WPF Sparkline control | SyncfusionÂ®
description: Learn how to get started with the SyncfusionÂ® WPF Sparkline control. Explore setup, features, examples, and customization options.
platform: chart-sdk
control: SfSparkline
documentation: ug
...
You need to initialize the sparkline represented by the WPF Line Sparkline class (`Syncfusion.UI.Xaml.Charts.SfLineSparkline`).
...
We need to add the above `UsersViewModel` to the `DataContext` of the sparkline, bind the data source to the `ItemsSource` property of the WPF Line Sparkline, and then map the data using `YBindingPath` and `XBindingPath`.
